...
hilo_pop <- get_acs(
  ...
  year = 2022,
  output = "wide",
  geometry = TRUE             # Include shapefiles for mapping
)

# Filter just for Hilo
hilo_only <- hilo_pop %>% filter(str_detect(NAME, "Hilo"))
...
```{r, message=FALSE, warning=FALSE, results='hide'}
hilo_interactive_map <- ggplot(hilo_only) +
  geom_sf_interactive(
    aes(
      fill = B01003_001E,
      tooltip = paste0(NAME, "\nPopulation: ", formatC(B01003_001E, format = "d", big.mark = ",")),
      data_id = GEOID
    ),
    color = "black",
    size = 0.3
  ...
  labs(
    title = "Total Population — Hilo CDP, Hawaii (ACS 2022)",
    fill = "Population"
  ) +
...
girafe(
  ggobj = hilo_interactive_map,
  width_svg = 7,
  height_svg = 5,
  options = list(
    opts_hover(css = "fill:#FF6600;cursor:pointer;"),
    opts_zoom(max = 5)
  )
)
...
hilo_time <- map_df(years, function(y) {
  get_acs(
    geography = "place", 
    ...
    filter(str_detect(NAME, "Hilo"))%>%
    mutate(year= y)
})

ggplot(hilo_time, aes(x = year, y = estimate, color = estimate)) +
  geom_line(linewidth = 1.5) +
  geom_point(size = 3) +
  scale_color_viridis_c(option = "plasma") +
  labs(
    title = "Total Population Over Time - Hilo CDP, Hawaii (ACS 2010–2022)",
    x = "Year",
    y = "Total Population (B01003_001)",
    color = "Population"
  ) +
...
hilo_total_pop_percent_pov <- get_acs(
  geography = "place",
...
ggplot(hilo_total_pop_percent_pov, aes(x = total_populationE,y = percent_povertyE)) +
  geom_point() +
  geom_smooth() + 
  stat_cor() + 
  theme_minimal() + 
  labs(title = "Relationship rates between hilo's population and its poverty percentage",
       subtitle = "2022 release",
       x = "Hio's Total Population",
       y = "Percent of Families Living in Poverty",
...
In this challenge, we used the 'B01003_001' code to find the total population of people in Hilo.
